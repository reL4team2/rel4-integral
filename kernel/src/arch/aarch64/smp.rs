...
pub fn handle_remote_call(call: ipi_remote_call, arg0: usize, arg1: usize, arg2: usize, irq_path: bool) {
    if crate::smp::clh_is_ipi_pending(cpu_id()) {
        match call {
            ...
            ipi_remote_call::IpiRemoteCall_InvalidateTranslationSingle => { invalidate_local_tlb_va_asid(arg0) }
            ipi_remote_call::IpiRemoteCall_InvalidateTranslationASID => { invalidate_local_tlb_asid(arg0) }
            ipi_remote_call::IpiRemoteCall_MaskPrivateInterrupt => {crate::interrupt::mask_interrupt(arg0 != 0, arg1)}
...
pub fn remote_invalidate_tlb_asid(asid: sel4_vspace::asid_t) {
    ...
    do_remote_mask_op(ipi_remote_call::IpiRemoteCall_InvalidateTranslationASID, asid as usize, 0, 0, mask);
}
...
pub fn remote_invalidate_translation_single(vptr: usize) {
    let mask = crate::BIT!(CONFIG_MAX_NUM_NODES) - 1;
    do_remote_mask_op(ipi_remote_call::IpiRemoteCall_InvalidateTranslationSingle, vptr, 0, 0, mask);
}

/// 和 seL4 有所不同，irq 就是 irq，不是 Index，方便 mask_interrupt
pub fn remote_mask_private_interrupt(cpu: usize, disable: bool, irq: usize) {
    do_remote_mask_op(ipi_remote_call::IpiRemoteCall_MaskPrivateInterrupt, disable as usize, irq, 0, cpu);
}